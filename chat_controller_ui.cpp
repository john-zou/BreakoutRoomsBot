	...
	if (chatMsg)
	{
		...
		if (chatMsg->IsChatToAll())
		{
			...
			if (pUser && (!pUser->IsMySelf()))
			{
				...
				std::wstring echoStr = std::wstring(L"echo : ") + content;
				SendChatMsg(0, const_cast<wchar_t*>(echoStr.c_str()));
			}
		}
		...
		else
		{
			...
			if(pUser&& !pUser->IsMySelf())
			{
				...
				BreakoutRoomBot::GetInstance().ProcessPrivateMessage(pUser, chatMsg);
			}
		}
	}
}
