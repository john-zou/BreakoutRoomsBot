...
void BreakoutRoomBot::StartBreakoutRooms() // TODO return enum
{
	if (areBreakoutRoomsEnabled()) {
		if (!SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->IsBOStarted()) {
			...
			auto started = SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->GetBOAdminHelper()->StartBO();
			if (started) {
				sendChatTo(0, L"StartBO returned true");
			}
			else {
				sendChatTo(0, L"StartBO returned false");
			}
		}
	}
}
...
void BreakoutRoomBot::ProcessPrivateMessage(ZOOM_SDK_NAMESPACE::IUserInfo * senderUserInfo, ZOOM_SDK_NAMESPACE::IChatMsgInfo * chatMessageInfo)
{
	...
	if (startsWith(message, "!host") || startsWith(message, "!kill")) {
		...
		auto adminHelper = SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->GetBOAdminHelper();
		BOOST_LOG_TRIVIAL(trace) << "!test -- GetBOAdminHelper: " << adminHelper;
		auto assistantHelper = SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->GetBOAssistantHelper();
		BOOST_LOG_TRIVIAL(trace) << "!test -- GetBOAssistantHelper: " << assistantHelper;
		auto creatorHelper = SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->GetBOCreatorHelper();
		BOOST_LOG_TRIVIAL(trace) << "!test -- GetBOCreatorHelper: " << creatorHelper;
		auto dataHelper = SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->GetBODataHelper();
		BOOST_LOG_TRIVIAL(trace) << "!test -- GetBODataHelper: " << dataHelper;
		auto attendee = SDKInterfaceWrap::GetInst().GetMeetingService()->GetMeetingBOController()->GetBOAttedeeHelper();
		BOOST_LOG_TRIVIAL(trace) << "!test -- GetBOAttedeeHelper: " << attendee;
	}
}
